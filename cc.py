...
class EnhancedProfileData(BaseModel):
    ...
    current_company_website: str = ""
    current_job_duration: str = ""
    current_job_duration_years: float = 0.0
...
class ProfileDataProcessor:
    ...
    def process_raw_profile(raw_data: Dict[str, Any]) -> EnhancedProfileData:
        ...
        skills = []
        skills_list = raw_data.get("skills", [])
        for skill in skills_list:
            if isinstance(skill, dict):
                skills.append({
                    "title": skill.get("title", ""),
                    "endorsements": skill.get("subComponents", []),
                    "context": skill.get("subComponents", [])
                })
        
        # Extract top skills
        top_skills = [skill.get("title", "") for skill in skills_list[:10] if skill.get("title")]
        
        # Process experiences with full details
        experiences = []
        exp_list = raw_data.get("experiences", [])
        for exp in exp_list:
            experiences.append({
                "title": exp.get("title", ""),
        ...
        # Process education
        education = []
        edu_list = raw_data.get("educations", [])
        for edu in edu_list:
            education.append({
        ...
        certifications = []
        cert_list = raw_data.get("licenseAndCertificates", [])
        for cert in cert_list:
            certifications.append({
                "title": cert.get("title", ""),
                "issuer": cert.get("subtitle", ""),
                "issue_date": cert.get("caption", ""),
                "credential_id": cert.get("metadata", ""),
                "logo": cert.get("logo", "")
            })
        
        # Process recent posts/updates
        recent_posts = []
        posts_list = raw_data.get("updates", [])
        ...
        return EnhancedProfileData(
            ...
            profile_pic=raw_data.get("profilePicHighQuality", raw_data.get("profilePic", "")),
            
            # Current Position
            current_job_title=raw_data.get("jobTitle", ""),
            current_company=raw_data.get("companyName", ""),
            current_company_industry=raw_data.get("companyIndustry", ""),
            current_company_website=raw_data.get("companyWebsite", ""),
            current_job_duration=raw_data.get("currentJobDuration", ""),
            current_job_duration_years=raw_data.get("currentJobDurationInYrs", 0.0),
